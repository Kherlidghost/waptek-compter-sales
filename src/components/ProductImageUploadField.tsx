...
export function ProductImageUploadField() {
  ...
  return (
    <div className="rounded-md border border-dashed border-slate-300 bg-slate-50 p-4 lg:col-span-2">
      <label className="grid cursor-pointer place-items-center rounded-md bg-surface p-6 text-center text-sm">
      ...
      {previews.length > 0 ? (
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {previews.map((preview, index) => (
            <div key={`${preview.name}-${index}`} className="rounded-md border border-slate-200 bg-surface p-2">
