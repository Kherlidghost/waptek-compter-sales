  ...
  return (
    <main className="min-h-screen space-y-6 bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.08),transparent_28%),linear-gradient(135deg,#f8fbff_0%,#f3f7fb_100%)] px-4 py-8 text-slate-900 sm:px-6 lg:px-8">
      ...
      <section className="mx-auto flex max-w-7xl flex-wrap items-end justify-between gap-4 rounded-[28px] border border-slate-200/80 bg-surface/90 p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] backdrop-blur">
        ...
        <a className="inline-flex items-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-emerald-500 hover:bg-emerald-50 hover:text-emerald-700" href="/admin/settings">
          ← Back to Settings
        </a>
      </section>

      {success ? <div className="mx-auto max-w-7xl rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">{success}</div> : null}
      {error ? <div className="mx-auto max-w-7xl rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-800">{error}</div> : null}
      {!authAdminReady ? (
        <div className="mx-auto max-w-7xl rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900">
        ...
        <form action={createStaffAccount} className="h-fit rounded-[28px] border border-slate-200/80 bg-surface/95 p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] backdrop-blur">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              Role
              <select className="wcs-input h-12" name="role" required defaultValue="manager">
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              Branch
              <select className="wcs-input h-12" name="branch_id" required defaultValue="">
            ...
            <button
              className="btn btn-primary h-12 justify-center rounded-2xl"
                      ...
                      <td className="px-4 py-3">
                        <details className="min-w-80 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                          ...
                          <form action={updateStaffAccount} className="mt-3 grid gap-3">
                            ...
                            <label className="grid gap-1 text-sm font-semibold text-slate-700">
                              Role
                              <select className="wcs-input h-11" name="role" defaultValue={item.role}>
                                {roles.map((userRole) => <option key={userRole} value={userRole}>{userRole}</option>)}
                              </select>
                            </label>
                            <label className="grid gap-1 text-sm font-semibold text-slate-700">
                              Branch
                              <select className="wcs-input h-11" name="branch_id" defaultValue={item.branch_id ?? ""}>
                            ...
                            <button className="btn btn-primary h-11 justify-center rounded-2xl">Save user</button>
                          </form>
